...
related: ["[[attention-mechanism.fr]]", "[[transformer-architecture.fr]]", "[[recurrent-networks-and-lstm.fr]]", "[[distillation.fr]]", "[[mixture-of-experts.fr]]"]
---
...
Pour générer le token $N+1$, [[attention-mechanism.fr|l'attention]] doit regarder tous les tokens $1 \ldots N$. Naïvement, il faut recalculer K et V pour chacun de ces tokens à chaque pas de génération → coût $O(n^2)$ par token, donc $O(n^3)$ pour générer $n$ tokens. Catastrophique sur de longs contextes.
