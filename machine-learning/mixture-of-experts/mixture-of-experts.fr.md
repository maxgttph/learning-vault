...
related: ["[[transformer-architecture.fr]]", "[[llm-inference-optimization.fr]]", "[[how-neural-networks-train.fr]]", "[[attention-mechanism.fr]]", "[[distillation.fr]]"]
---
...
Pas d'étiquettes « ce token doit aller à l'expert 4 », pas d'entraînement séparé. Le routeur est appris **end-to-end par la même backprop** que le reste du modèle (voir [[how-neural-networks-train.fr]]).

Le ressort : comme la sortie des experts choisis est **multipliée par les poids softmax du routeur**, le gradient remonte jusqu'à $W_{\text{router}}$. Si l'expert 2 a produit un résultat utile, la perte baisse, et le gradient pousse $W_{\text{router}}$ à donner un score plus élevé à l'expert 2 pour des tokens similaires. La **spécialisation est émergente, jamais assignée** — exactement la dynamique de divergence forcée décrite dans [[attention-mechanism.fr]] (les « trois ouvriers identiques » qui se spécialisent par leur position dans la chaîne).
...
MoE déplace le goulot d'étranglement exactement dans le régime décrit par [[llm-inference-optimization.fr]] : l'inférence est **bornée par la mémoire, pas par le compute**.
...
- Peut-on **distiller** un gros MoE vers un modèle dense plus petit, et que perd-on ? (voir [[distillation.fr]])
