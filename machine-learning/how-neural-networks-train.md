...
related: ["[[neural-networks-overview]]", "[[transformer-architecture]]", "[[attention-mechanism]]"]
---
...
- **Gradient descent** — `w ← w − η · ∂L/∂w`. Cœur de l'apprentissage.
...
5. **Update** — l'optimizer applique le pas. Avec SGD : `w ← w − η · ∂L/∂w`. Avec Adam : idem + momentum + learning rate adaptatif par paramètre.
...
**État initial :** `W₁ = [[0.1, 0.2], [0.3, 0.4]]`, `W₂ = [0.5, 0.6]`, biais = 0.
Exemple : `x = [1, 2]`, cible `y = 1`.

**Forward :**
- `z₁ = [0.5, 1.1]` → `h = ReLU(z₁) = [0.5, 1.1]`
- `z₂ = 0.5·0.5 + 0.6·1.1 = 0.91` → `y_pred = sigmoid(0.91) ≈ 0.713`
- **Loss (BCE)** = `−ln(0.713) ≈ 0.338`

**Backward** (astuce : pour sigmoid + BCE, `∂L/∂z₂ = y_pred − y = −0.287` se simplifie magnifiquement) :
- Propage en remontant via la règle de la chaîne.
- Donne un gradient pour chaque poids individuel.

**Update** avec `η = 0.1` : tous les poids bougent légèrement dans la direction qui réduit la loss.

**Vérification :** refait le forward avec les nouveaux poids → `y_pred ≈ 0.748`, loss ≈ 0.290. La prédiction s'est rapprochée de la cible 1. Multiplié à 70 milliards de paramètres et `10¹³` tokens, c'est exactement cette mécanique qui produit un LLM moderne.
...
- **Sigmoid + BCE.** Le couple est utilisé partout parce que leur dérivée combinée se simplifie en `y_pred − y_true`. Pas de hasard : c'est précisément cette élégance qui les rend pratiques.
