...
translations:
  - "[[normalization.en]]"
related: ["[[bias.fr]]", "[[transformer-architecture.fr]]", "[[attention-mechanism.fr]]", "[[how-neural-networks-train.fr]]"]
---
...
- **Absorption du biais** — la normalisation par re-centrage annule mathématiquement tout [[bias.fr|biais]] ajouté avant elle, rendant ces biais redondants.
...
Dans pre-norm : une LayerNorm par sous-bloc (donc 2 par couche [[transformer-architecture.fr|Transformer]] — une avant l'[[attention-mechanism.fr|attention]], une avant le MLP), plus une **LayerNorm finale** avant l'un-embedding.

### Comment la normalisation absorbe le rôle du biais

C'est la connexion clé avec [[bias.fr|le biais]].
